...
When a character portrait has been generated via the avatar generator
(Section~\ref{sec:charcreate}), it is displayed in the first column of the
character sheet beneath the combat statistics, providing a visual
representation of the active character during play.
...
\subsubsection*{Avatar Generator}
\label{sec:avgen}

The \emph{Create Character} tab additionally hosts a built-in
\textbf{character avatar generator} powered by \textbf{SDXL-Turbo}
(Stable Diffusion XL Turbo by Stability AI)~\cite{sauer2024adversarial}, a distilled
text-to-image model that produces 512$\times$512 pixel character
portraits in as few as four inference steps using
\texttt{guidance\_scale\,=\,0.0}. The implementation is split between
\texttt{web/handlers/avatar\_handlers.py} (Gradio integration with lazy
pipeline caching) and \texttt{scripts/create\_avatar.py} (a standalone
interactive CLI offering the same functionality).

\paragraph{Generation modes.}
Two modes are selectable via radio button:

\begin{description}[noitemsep]
    \item[Auto (from character stats)] assembles a Stable Diffusion prompt
        automatically from the character's race, class, alignment, and
        background using four lookup dictionaries---\texttt{RACE\_KEYWORDS},
        \texttt{CLASS\_KEYWORDS}, \texttt{ALIGNMENT\_KEYWORDS}, and
        \texttt{BACKGROUND\_KEYWORDS}---that map D\&D attribute values to
        descriptive visual phrases. For example, a Tiefling Warlock with a
        Chaotic Neutral alignment produces the fragments
        \emph{``tiefling, small curved horns, solid-colored eyes, long tail,
        infernal heritage''}, \emph{``dark robes, eldritch glow, mysterious,
        pact focus''}, and \emph{``unpredictable smirk, wild eyes, eccentric''}.
        A fixed style suffix (\emph{fantasy portrait, D\&D 5e art style, highly
        detailed, dramatic lighting, digital painting}) is appended to all
        auto-generated prompts.
    \item[Custom] reveals additional form fields for gender presentation,
        hair colour and style, eye colour, skin tone, expression/mood,
        clothing details, environment/background, and one of five art-style
        presets: \emph{bright heroic (classic D\&D art)},
        \emph{realistic painterly (dark fantasy)},
        \emph{anime / manga inspired}, \emph{gritty grimdark}, or
        \emph{watercolor storybook}. Race and class remain fixed from the
        character data; user-supplied fields are placed at the front of the
        assembled prompt so they are preserved if token trimming is required.
\end{description}

\paragraph{CLIP token trimming.}
SDXL-Turbo's CLIP text encoder accepts at most 77~tokens. The
\texttt{clip\_trim()} helper tokenises the assembled prompt and drops
comma-separated phrases from the right until the token count falls within the
limit, ensuring that character-specific descriptors near the front are always
retained.

\paragraph{Pipeline and persistence.}
The Stable Diffusion pipeline (\texttt{Auto\-Pipeline\-For\-Text2\-Image} from
the \texttt{diffusers} library) is lazy-loaded on first generation---downloading
approximately 6\,GB of model weights on the initial run---and then cached in
memory (\texttt{\_pipe} module-level variable) for all subsequent generations
within the same Gradio session. Portraits are saved as PNG files under
\texttt{scripts/characters/portraits/<name>.png}. The file path is written
back into the character's JSON file under the \texttt{image\_path} key, so
the portrait persists across sessions and is automatically loaded into the
character sheet whenever that character is selected for play.
