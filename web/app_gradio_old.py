...
def load_character_with_location(character_choice: str, location_name: Optional[str] = None) -> Tuple[str, str, str]:
    ...
    if char.spells:
        ...
        # Load spells into known_spells list (for spell casting system)
        char_state.known_spells = list(char.spells)

        # AUTO-PREPARE SPELLS (reduce spell accounting!)
        if char_state.spellcasting_class:
            if char_state.is_prepared_caster():
                # Wizards, Clerics, Druids, Paladins - auto-prepare useful spells
                ...
                # Smart auto-prepare: prioritize combat spells, then utility
                ...
                # Sort spells: combat first, then utility, then alphabetical
                ...
                # Auto-prepare up to limit
                char_state.prepared_spells = sorted_spells[:max_prepared]
                print(f"🎯 Auto-prepared {len(char_state.prepared_spells)} spells for {char.name}: {', '.join(char_state.prepared_spells[:3])}{'...' if len(char_state.prepared_spells) > 3 else ''}")
            else:
                # Sorcerers, Bards, Warlocks - all spells always prepared
...
def load_character(character_choice: str) -> Tuple[str, str, str, str, list, Optional[str]]:
...
*Type `/help` to see available commands, or describe your action!*"""
    ...
    # Return character sheet (3 columns), clear input, chat with welcome message, and image
    initial_chat = [{"role": "assistant", "content": welcome_message}]
    col1, col2, col3 = format_character_sheet()
    return col1, col2, col3, "", initial_chat, char_image


def load_character_with_debug(character_choice: str, scenario_choice: Optional[str]) -> Tuple[str, str, str, str, list, Optional[str]]:
    """
...
def load_party_mode() -> Tuple[str, str, str, gr.update, list]:
...
def format_character_sheet() -> tuple:
    """
    Format character sheet for 3-column display.

    Returns:
        Tuple of (col1, col2, col3) markdown strings
        - col1: Name, combat stats, ability scores
        - col2: Equipment and inventory
...
| STR | DEX | CON |
|:---:|:---:|:---:|
| {char.strength}<br>({mods['strength']:+d}) | {char.dexterity}<br>({mods['dexterity']:+d}) | {char.constitution}<br>({mods['constitution']:+d}) |

| INT | WIS | CHA |
|:---:|:---:|:---:|
| {char.intelligence}<br>({mods['intelligence']:+d}) | {char.wisdom}<br>({mods['wisdom']:+d}) | {char.charisma}<br>({mods['charisma']:+d}) |
...
def handle_next_turn(history: list) -> Tuple[list, str, gr.update, str, str, str]:
    """
...
def handle_end_combat(history: list) -> Tuple[list, str, gr.update, str, str, str]:
    """
...
def get_current_sheet() -> tuple:
    ...
    Returns:
        Tuple of (col1, col2, col3) for character mode
        or (col1_party, "", "") for party mode (party mode uses single column)
    """
...
def chat(message: str, history: list) -> Tuple[list, str, gr.update, str, str, str]:
    """
...
with demo:
        ...
        with gr.Tab("🎮 Play Game"):
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    # 3-Column Character Sheet Layout
                    with gr.Row():
                        with gr.Column(scale=1):
                            char_col1 = gr.Markdown("No character loaded")
                        with gr.Column(scale=1):
                            char_col2 = gr.Markdown("")
                        with gr.Column(scale=1):
                            char_col3 = gr.Markdown("")

                    # RAG Lookup Panel for Spells & Items
                    with gr.Accordion("📖 Quick Spell & Item Lookup", open=False) as rag_lookup_accordion:
                        ...
                        with gr.Row():
                            rag_lookup_input = gr.Textbox(
                                placeholder="Enter spell or item name (e.g., 'Magic Missile', 'Longsword')",
                                label="Spell/Item Name",
                                scale=3
                            )
                            rag_lookup_btn = gr.Button("🔍 Lookup", variant="primary", scale=1)

                        rag_lookup_output = gr.Markdown("*Enter a spell or item name above to see details*")
    ...
    load_btn.click(
        ...
        outputs=[char_col1, char_col2, char_col3, msg_input, chatbot, char_image]
    )

    load_party_btn.click(
        load_party_mode,
        inputs=[],
        outputs=[char_col1, char_col2, char_col3, msg_input, chatbot]
    )
    ...
    # RAG Lookup handler
    def handle_rag_lookup(query: str) -> str:
        """
...
"""
    ...
    rag_lookup_btn.click(
        handle_rag_lookup,
        inputs=[rag_lookup_input],
        outputs=[rag_lookup_output]
    )
    ...
    submit_btn.click(
        ...
        outputs=[chatbot, initiative_display, combat_accordion, char_col1, char_col2, char_col3]
    ).then(
        lambda: "",
        outputs=[msg_input]
    )
